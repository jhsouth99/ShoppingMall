/**
 * Social Signup Page JavaScript
 * 소셜 회원가입 페이지 기능
 */

// 전역 변수
let nameValid = true; // 소셜에서 가져온 경우 기본적으로 valid
...
function updateEmailVerificationUI() {
    ...
    if (socialEmail && currentEmail === socialEmail) {
        // 소셜 이메일 그대로 사용하는 경우
        $('#sendMailBtn').hide();
        $('.email-verify-group').hide();
        emailVerified = true;
        $('#emailFeedback').text('소셜 계정 이메일입니다. 중복 확인을 눌러주세요.').addClass('valid');
    } else if (currentEmail !== socialEmail) {
        // 이메일을 변경한 경우
        $('#sendMailBtn').show();
        $('.email-verify-group').show();
        ...
        emailVerified = false;
        $('#emailCode').val('');
        $('#emailFeedback').text('중복 확인을 눌러주세요.').removeClass('valid invalid');
        $('#emailCheckFeedback').text('').removeClass('valid invalid');
    }
...
function setupEventListeners() {
    ...
    // 이메일 수정 시
    $('#email').on('input', updateEmailVerificationUI);

    // 폼 제출
    $('#socialSignupForm').on('submit', handleFormSubmit);
}
...
function toggleSubmit() {
    const allValid = nameValid && usernameValid && passwordMatch && phoneValid &&
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                $submitBtn.prop('disabled', false).text('가입 완료하기');
            }
        },
        error: function(xhr) {
...
function startEmailTimer() {
    ...
    const timer = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            ...
            $('#sendMailBtn').prop('disabled', false);
            $('#emailCode').prop('disabled', true).val('');
            $('#checkMailBtn').prop('disabled', true);
        }
