...
function initBannerSlider() {
    ...
    nextBtn.addEventListener('click', () => { stopSlideInterval(); nextSlide(); startSlideInterval(); });
    prevBtn.addEventListener('click', () => { stopSlideInterval(); prevSlide(); startSlideInterval(); });
    dots.forEach((dot, idx) => dot.addEventListener('click', () => { stopSlideInterval(); showSlide(idx); startSlideInterval(); }));
...
function initSearchFunctionality() {
    ...
    function renderHistory() {
        ...
        items.forEach((term, idx) => {
            ...
            del.className = 'delete-search-item'; del.textContent = '×'; del.dataset.idx = idx;
            del.addEventListener('click', e => { e.stopPropagation(); items.splice(idx,1); localStorage.setItem('searchHistory', JSON.stringify(items)); renderHistory(); });
            li.append(span, del);
    ...
    function performSearch(term) {
        ...
        items.unshift(term); if(items.length>10) items.pop();
        localStorage.setItem('searchHistory', JSON.stringify(items)); renderHistory();
    }

    input.addEventListener('focus', () => { hist.style.display = 'block'; renderHistory(); });
    document.addEventListener('click', e => {
        if (!input.contains(e.target) && !hist.contains(e.target) && !btn.contains(e.target)) {
...
function initMenuHoverEffects() {
    document.querySelectorAll('.has-submenu').forEach(item => {
        item.addEventListener('touchstart', e => {
            e.preventDefault();
            document.querySelectorAll('.submenu.active').forEach(sm => { if (!item.contains(sm)) sm.classList.remove('active'); });
...
function initProductFiltering() {
    ...
    [cat,price,disc,grp,sort].forEach(el => el.addEventListener('change', apply));
    function apply() {
        console.log('필터 변경', {cat:cat.value,price:price.value,disc:disc.checked,grp:grp.checked,sort:sort.value});
        document.querySelectorAll('.product-card').forEach(card => {
            let show=true;
            if(price.value!=='all'){
                const p = parseInt((card.querySelector('.discount-price')||card.querySelector('.current-price')).textContent.replace(/\D/g,''));
                if(price.value==='under-100000'&&p>100000) show=false;
                if(price.value==='100000-300000'&&(p<100000||p>300000)) show=false;
                if(price.value==='over-300000'&&p<300000) show=false;
            }
            if(disc.checked && !card.querySelector('.product-badge.discount')) show=false;
            if(grp.checked && !card.querySelector('.group-purchase-info')) show=false;
